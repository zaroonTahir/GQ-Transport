...
const GallerySection = () => {
  ...
  return (
    <>
      <section id="gallery" className="pt-12 pb-24 bg-gradient-to-b from-gray-950 via-gray-900 to-gray-950 relative overflow-hidden" ref={sectionRef}>
        ...
        <div className="container mx-auto px-4 relative z-10">
              ...
              >
                ...
                <div className="absolute inset-0 flex flex-col justify-end p-6">
                  <div className="translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
                    <div className={`inline-block px-3 py-1 bg-gradient-to-r ${image.gradient} rounded-full text-xs font-semibold text-white mb-3 opacity-0 group-hover:opacity-100 transition-opacity shadow-lg`}>
                      {image.category}
                    </div>
                    <h3 className="text-white text-xl font-bold">{image.title}</h3>
                  </div>

                  <div className="absolute top-4 right-4 w-10 h-10 bg-white/10 backdrop-blur-sm rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all border border-white/20">
          ...
          {/* Stats Bar */}
          <div
            className={`mt-16 bg-gray-800/50 backdrop-blur-sm rounded-2xl shadow-2xl p-8 border border-white/10 transform transition-all duration-1000 delay-500 ${
              isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
            }`}
          >
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
              {[
                ...
                <div key={i} className="group">
                  <div className={`text-4xl font-bold mb-2 bg-gradient-to-r ${stat.gradient} bg-clip-text text-transparent group-hover:scale-110 transition-transform duration-300`}>
                    {stat.value}
                  </div>
      ...
      {lightboxOpen && (
        <div className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center animate-fadeIn">
          <button onClick={closeLightbox} className="absolute top-6 right-6 w-12 h-12 bg-gray-800/50 backdrop-blur-sm hover:bg-gray-700/50 border border-white/10 rounded-full flex items-center justify-center transition-all">
            <X className="w-6 h-6 text-white" />
          </button>

          <button onClick={prevImage} className="absolute left-6 w-12 h-12 bg-gray-800/50 backdrop-blur-sm hover:bg-gray-700/50 border border-white/10 rounded-full flex items-center justify-center transition-all">
            <ChevronLeft className="w-6 h-6 text-white" />
          </button>

          <button onClick={nextImage} className="absolute right-6 w-12 h-12 bg-gray-800/50 backdrop-blur-sm hover:bg-gray-700/50 border border-white/10 rounded-full flex items-center justify-center transition-all">
          ...
          <div className="max-w-6xl max-h-[90vh] mx-4">
            ...
            <div className="text-center mt-6">
              <div className={`inline-block px-4 py-2 bg-gradient-to-r ${galleryImages[currentImage].gradient} rounded-full mb-2 shadow-lg`}>
                <span className="text-sm text-white font-semibold">
                  {galleryImages[currentImage].category}
                </span>
              </div>
              <h3 className="text-white text-2xl font-bold mb-2 bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">
